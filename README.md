...
## v1.4 — FAST FAIL / FAST RETRY

プレイ時間を「一面を長く粘る」方向から、「短い試行を高速で繰り返す」方向へ変更。

- 固定24面の制限時間を約8〜11秒へ短縮
- ENDLESSは約6〜9秒の短い試行
- GOALへ入れば即CLEAR
- ゲートへ接触した瞬間FAIL
- 外壁へ接触した瞬間FAIL
- FAIL後は結果画面を挟まず約0.52秒で同じCHAMBERを自動再開
- CLEAR後の次面移行も約0.34秒へ短縮
- 通常ゲート幅を大幅に拡大し、通過そのものより「正しい軌道を作れるか」を難しさの中心へ移動
- ENDLESSのゲートも最低約146px相当を確保し、極端な狭さで難度を作らない
- 自動リトライは小さなスコア減算だけで、失敗からすぐ学べる

狙いは **成功なら数秒で次へ、失敗なら数秒以内に即死して同じ問題へ再挑戦** するテンポ。
